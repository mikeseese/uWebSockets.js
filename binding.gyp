{
  "variables": {
    "target_node_module_version": "<!(node -p \"require('fs').readFileSync(require('path').join('<(nodedir)'.replace('\\', '\\\\'), 'include', 'node', 'node_version.h'), { encoding: 'utf8' }).match(/#define NODE_MODULE_VERSION ([0-9]+)/)[1]\")",
